...
import type { PartnerProfile, PartnerLevel } from '../../types/AdminTypes';
...
export function PartnerManager() {
  ...
  const handleMove = (index: number, direction: 'up' | 'down') => {
    ...
    const reorderedIds = newPartners.map(p => p.id);
    mockAdminService.reorderPartners(reorderedIds);
    loadPartners();
  };

  const startEdit = (partner: PartnerProfile) => {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && editingPartner) {
      ...
      reader.onloadend = () => {
        setEditingPartner({ ...editingPartner, image: reader.result as string });
      };
            ...
            <form onSubmit={handleSave} className="admin-form">
                ...
                <div className="admin-image-upload">
                  {editingPartner.image && (
                    <div className="admin-image-preview">
                      <img src={editingPartner.image} alt="Preview" />
                      <button type="button" onClick={() => setEditingPartner({...editingPartner, image: null})}>
                        Remove
                      </button>
                    </div>
                  )}
              ...
              <div className="admin-form__field">
                ...
                <select 
                  ...
                  onChange={e => setEditingPartner({...editingPartner, badge: e.target.value as PartnerLevel})}
                >
                  <option value="none">None</option>
                  <option value="gold">Gold (Leader)</option>
                  <option value="blue">Blue (Partner)</option>
              ...
              <div className="admin-form__field">
                <div className="admin-label-row">
                  ...
                  <span className={`admin-char-count ${editingPartner.desc.length > 200 ? 'is-warning' : ''}`}>
                    {editingPartner.desc.length}/250
                  </span>
                </div>
                <textarea 
                  value={editingPartner.desc} 
                  onChange={e => setEditingPartner({...editingPartner, desc: e.target.value.slice(0, 250)})}
                  rows={3}
                  required
                />
              </div>
              ...
              <div className="admin-form__field">
                <div className="admin-label-row">
                  ...
                  <span className={`admin-char-count ${editingPartner.descKo.length > 200 ? 'is-warning' : ''}`}>
                    {editingPartner.descKo.length}/250
                  </span>
                </div>
                <textarea 
                  value={editingPartner.descKo} 
                  onChange={e => setEditingPartner({...editingPartner, descKo: e.target.value.slice(0, 250)})}
                  rows={3}
                  required
                />
              </div>
                ...
                <div className="admin-form__flex-row">
                  <div className="admin-form__field">
                    ...
                    <div className="admin-input-with-icon">
                      ...
                      <input 
                        type="text" 
                        value={editingPartner.socials.twitter || ''} 
                        onChange={e => setEditingPartner({
                          ...editingPartner, 
                          socials: { ...editingPartner.socials, twitter: e.target.value }
                        })}
                      />
                    </div>
                  </div>
                  <div className="admin-form__field">
                    ...
                    <div className="admin-input-with-icon">
                      ...
                      <input 
                        ...
                        onChange={e => setEditingPartner({
                          ...editingPartner, 
                          socials: { ...editingPartner.socials, linkedin: e.target.value }
                        })}
                      />
                    </div>
                  </div>
                  <div className="admin-form__field">
                    ...
                    <div className="admin-input-with-icon">
                      ...
                      <input 
                        ...
                        onChange={e => setEditingPartner({
                          ...editingPartner, 
                          socials: { ...editingPartner.socials, instagram: e.target.value }
                        })}
                      />
                    </div>
                  </div>
        ...
        {partners.map((partner, index) => (
          <div key={partner.id} className="partner-list-item">
            ...
            <div className="col-info">
              <div className="partner-info-cell">
                <div className="partner-info-cell__avatar">
                  {partner.image ? (
                    <img src={partner.image} alt="" className="admin-avatar-img" />
                  ) : (
            ...
            <div className="col-level">
              <span className={`badge-pill badge-pill--${partner.badge}`}>
                {partner.badge}
              </span>
            </div>
            ...
            <div className="col-actions">
              <div className="action-btns">
                <button className="action-btn action-btn--edit" onClick={() => startEdit(partner)}>
                  <Edit2 size={16} />
                </button>
