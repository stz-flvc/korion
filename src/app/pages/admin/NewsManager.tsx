...
import type { CardNews, NewsCategory } from '../../types/AdminTypes';
...
export function NewsManager() {
  ...
  const handleMove = (index: number, direction: 'up' | 'down') => {
    ...
    [newNews[index], newNews[targetIndex]] = [newNews[targetIndex], newNews[index]];
    
    const reorderedIds = newNews.map(n => n.id);
            ...
            <form onSubmit={handleSave} className="admin-form">
              ...
              <div className="admin-form__flex-row">
                <div className="admin-checkbox-field">
                  ...
                  />
                  <label htmlFor="featured">Featured (Top Section)</label>
                </div>
                <div className="admin-checkbox-field">
                  <input 
                    type="checkbox" 
                  ...
                  />
                  <label htmlFor="pinned">Pinned (Notice List)</label>
                </div>
              </div>
          ...
          <div className="news-grid">
            {news.map((item) => (
              <div key={item.id} className="news-admin-card">
                <div className="news-admin-card__banner">
                  <span className={`news-cat-tag news-cat-tag--${item.category}`}>
                    {item.category}
                  </span>
                  <div className="news-admin-card__flags">
                    {item.featured && <Sparkles size={14} className="icon-featured" />}
                    {item.pinned && <Pin size={14} className="icon-pinned" />}
                  </div>
                </div>
                
                <div className="news-admin-card__body">
                  <div className="news-admin-card__order">
                    <button disabled={news.findIndex(n => n.id === item.id) === 0} onClick={() => handleMove(news.findIndex(n => n.id === item.id), 'up')}><ArrowUp size={14}/></button>
                    <span>{news.findIndex(n => n.id === item.id) + 1}</span>
                    <button disabled={news.findIndex(n => n.id === item.id) === news.length - 1} onClick={() => handleMove(news.findIndex(n => n.id === item.id), 'down')}><ArrowDown size={14}/></button>
                  </div>
                  <h3>{item.title}</h3>
                ...
                <div className="news-admin-card__footer">
                  <button 
                    className={`status-chip ${item.isVisible ? 'is-visible' : ''}`}
                    onClick={() => {
